-- 116_dashboard_metrics_pending_split.sql
-- Substitui a coluna única "pending" (não era limitada ao mês de referência,
-- o que impedia bater a conta com Total/Realizadas) por duas: pending_month
-- (pendências cuja data cai dentro do mês de referência) e pending_previous
-- (pendências com data anterior ao mês de referência — atraso acumulado).
-- Mesma assinatura da migration 114 — apenas o corpo muda (CREATE OR REPLACE).
