-- 81_audit_series.sql
-- Adiciona audit triggers nas tabelas de recorrência introduzidas na etapa 6:
--   consultation_series  — resource_name: "Paciente | Especialidade | Início: DD/MM/YYYY"
--   consultation_therapists — resource_name: "Paciente | DD/MM/YYYY | Terapeuta"
-- Recria fn_audit_log com os novos ELSIF antes do bloco de fallback genérico.

CREATE OR REPLACE FUNCTION fn_audit_log()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
SET row_security = off
AS $$
DECLARE
  v_resource_name TEXT := '';
  v_resource_id   UUID;
  v_action        TEXT;
  v_user_id       UUID := NULL;
  v_user_email    TEXT := '';
  v_user_name     TEXT := '';
  v_rec           RECORD;
  v_mr_label      TEXT := '';
BEGIN
  -- Extrai user_id do JWT
  BEGIN
    v_user_id := (current_setting('request.jwt.claims', true)::jsonb ->> 'sub')::uuid;
  EXCEPTION WHEN OTHERS THEN
    v_user_id := NULL;
  END;

  -- Busca email e nome do usuário
  IF v_user_id IS NOT NULL THEN
    BEGIN
      SELECT email INTO v_user_email FROM auth.users WHERE id = v_user_id;
    EXCEPTION WHEN OTHERS THEN
      v_user_email := '';
    END;
    BEGIN
      SELECT name INTO v_user_name FROM therapists WHERE user_id = v_user_id LIMIT 1;
    EXCEPTION WHEN OTHERS THEN
      v_user_name := '';
    END;
    IF COALESCE(v_user_name, '') = '' THEN
      BEGIN
        SELECT COALESCE(raw_user_meta_data->>'full_name', raw_user_meta_data->>'name', '')
        INTO v_user_name FROM auth.users WHERE id = v_user_id;
      EXCEPTION WHEN OTHERS THEN
        v_user_name := '';
      END;
    END IF;
    IF COALESCE(v_user_name, '') = '' THEN
      v_user_name := v_user_email;
    END IF;
  END IF;

  -- Define action e registro de referência
  IF TG_OP = 'DELETE' THEN
    v_action := 'DELETE'; v_rec := OLD;
  ELSIF TG_OP = 'INSERT' THEN
    v_action := 'INSERT'; v_rec := NEW;
  ELSE
    v_action := 'UPDATE'; v_rec := NEW;
  END IF;

  v_resource_id := v_rec.id;

  -- ── Resource name por tabela ────────────────────────────────
  IF TG_TABLE_NAME = 'consultations' THEN
    BEGIN
      SELECT
        'Paciente: ' || p.full_name
        || ' | Data/Hora: ' || to_char(v_rec.date, 'DD/MM/YYYY')
        || CASE WHEN v_rec.time IS NOT NULL AND v_rec.time <> '' THEN ' ' || v_rec.time ELSE '' END
        || ' | Especialidade: ' || COALESCE(v_rec.specialty, '—')
        || ' | Terapeuta: ' || COALESCE(t.name, '—')
        || CASE WHEN v_rec.appointment_type_id IS NOT NULL THEN
              ' | Tipo: ' || COALESCE((SELECT name FROM appointment_types WHERE id = v_rec.appointment_type_id), '—')
           ELSE '' END
        || CASE WHEN v_rec.room_id IS NOT NULL THEN
              ' | Sala: ' || COALESCE((SELECT name FROM rooms WHERE id = v_rec.room_id), '—')
           ELSE '' END
        || CASE WHEN v_rec.consultation_status_id IS NOT NULL THEN
              ' | Status: ' || COALESCE((SELECT name FROM consultation_statuses WHERE id = v_rec.consultation_status_id), '—')
           ELSE '' END
      INTO v_resource_name
      FROM   patients   p
      JOIN   therapists t ON t.id = v_rec.therapist_id
      WHERE  p.id = v_rec.patient_id;
      IF v_resource_name IS NULL THEN
        v_resource_name := COALESCE(to_char(v_rec.date, 'DD/MM/YYYY'), '');
      END IF;
    EXCEPTION WHEN OTHERS THEN
      BEGIN v_resource_name := to_char(v_rec.date, 'DD/MM/YYYY'); EXCEPTION WHEN OTHERS THEN v_resource_name := ''; END;
    END;

  ELSIF TG_TABLE_NAME IN ('medical_record_exams','medical_record_medications','medical_record_conducts') THEN
    v_mr_label := CASE TG_TABLE_NAME
      WHEN 'medical_record_exams'        THEN 'Exames'
      WHEN 'medical_record_medications'  THEN 'Medicamentos'
      WHEN 'medical_record_conducts'     THEN 'Conduta'
      ELSE TG_TABLE_NAME
    END;
    BEGIN
      SELECT p.full_name || ' | ' || v_mr_label
      INTO   v_resource_name
      FROM   medical_records mr
      JOIN   patients p ON p.id = mr.patient_id
      WHERE  mr.id = v_rec.medical_record_id;
      IF v_resource_name IS NULL THEN v_resource_name := v_mr_label; END IF;
    EXCEPTION WHEN OTHERS THEN
      v_resource_name := v_mr_label;
    END;

  ELSIF TG_TABLE_NAME = 'consultation_series' THEN
    BEGIN
      SELECT p.full_name || ' | ' || COALESCE(v_rec.specialty, '') || ' | Início: ' || to_char(v_rec.start_date, 'DD/MM/YYYY')
      INTO v_resource_name
      FROM patients p
      WHERE p.id = v_rec.patient_id;
      IF v_resource_name IS NULL THEN
        v_resource_name := 'Série ' || COALESCE(to_char(v_rec.start_date, 'DD/MM/YYYY'), '');
      END IF;
    EXCEPTION WHEN OTHERS THEN
      v_resource_name := 'Série';
    END;

  ELSIF TG_TABLE_NAME = 'consultation_therapists' THEN
    BEGIN
      SELECT p.full_name || ' | ' || to_char(c.date, 'DD/MM/YYYY') || ' | ' || t.name
      INTO v_resource_name
      FROM consultations c
      JOIN patients   p ON p.id = c.patient_id
      JOIN therapists t ON t.id = v_rec.therapist_id
      WHERE c.id = v_rec.consultation_id;
      IF v_resource_name IS NULL THEN v_resource_name := 'Participante'; END IF;
    EXCEPTION WHEN OTHERS THEN
      v_resource_name := 'Participante';
    END;

  ELSE
    -- Fallback genérico: full_name → name → label → date
    BEGIN v_resource_name := v_rec.full_name;
    EXCEPTION WHEN undefined_column THEN v_resource_name := ''; END;

    IF COALESCE(v_resource_name, '') = '' THEN
      BEGIN v_resource_name := v_rec.name;
      EXCEPTION WHEN undefined_column THEN v_resource_name := ''; END;
    END IF;

    IF COALESCE(v_resource_name, '') = '' THEN
      BEGIN v_resource_name := v_rec.label;
      EXCEPTION WHEN undefined_column THEN v_resource_name := ''; END;
    END IF;

    IF COALESCE(v_resource_name, '') = '' THEN
      BEGIN v_resource_name := v_rec.date::TEXT;
      EXCEPTION WHEN undefined_column THEN v_resource_name := ''; END;
    END IF;
  END IF;

  -- Insere o log
  BEGIN
    INSERT INTO audit_logs (user_id, user_email, user_name, action, resource_type, resource_id, resource_name)
    VALUES (v_user_id, COALESCE(v_user_email,''), COALESCE(v_user_name,''), v_action, TG_TABLE_NAME, v_resource_id, COALESCE(v_resource_name,''));
  EXCEPTION WHEN OTHERS THEN
    -- não quebra a operação principal
  END;

  IF TG_OP = 'DELETE' THEN RETURN OLD; ELSE RETURN NEW; END IF;
END;
$$;

-- ── Triggers para as novas tabelas ───────────────────────────

DROP TRIGGER IF EXISTS trg_audit_consultation_series ON consultation_series;
CREATE TRIGGER trg_audit_consultation_series
  AFTER INSERT OR UPDATE OR DELETE ON consultation_series
  FOR EACH ROW EXECUTE FUNCTION fn_audit_log();

DROP TRIGGER IF EXISTS trg_audit_consultation_therapists ON consultation_therapists;
CREATE TRIGGER trg_audit_consultation_therapists
  AFTER INSERT OR UPDATE OR DELETE ON consultation_therapists
  FOR EACH ROW EXECUTE FUNCTION fn_audit_log();
