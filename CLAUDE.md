...
- **Status inicial de novo atendimento** (`ConsultationFormModal`, `!isEdit`): prioriza o status com `is_scheduling_default = true`; se nenhum estiver configurado, cai no status `is_awaiting_outcome = true` ativo alfabeticamente primeiro (migration 118 — antes era busca textual por `name.includes('agendada')`, último resquício de busca por nome migrado para flag).
