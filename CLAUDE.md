...
- **`src/utils/dashboardMetrics.js`** — helpers puros, única fonte para as duas visões:
  ...
  - `computePodiumProjection({ completed, total, pending, thirdPlaceEntry })` — quantos preenchimentos faltam para **entrar no pódio** (3º lugar do ranking), não para uma faixa arbitrária de 85/95% (a meta real é sempre 100%; o pódio é só o próximo marco acionável no caminho até lá). Simula `completed + x` (x de 1 até `pending`) e usa o **mesmo** `compareTherapistPerformance` do ranking pra achar o menor `x` que empata ou supera `thirdPlaceEntry` — "entrar no pódio" aqui significa exatamente aparecer entre os 3 primeiros na tabela, nada de fórmula paralela. **Sem prometer o impossível**: se nem preenchendo todas as `pending` bastar, `achievable: false` e a UI cai numa mensagem genérica. Se `thirdPlaceEntry` for `null` (menos de 3 terapeutas elegíveis) ou o usuário já estiver no pódio, `onPodium: true` direto. Pode ultrapassar a 3ª posição com um número inteiro de preenchimentos — aceito por design, a mensagem não promete "exatamente 3º".
...
- **`MyPerformanceCard`** (idem, só na view terapeuta; recebe `thirdPlaceEntry = rankingSorted[2]` calculado em `DashboardPage.jsx`): Taxa, Posição (índice+1 na lista ordenada/filtrada da RPC), "{completed} de {total}" (rótulo **"Atendidos"**), Pendências (`pendingMonth + pendingPrevious`, com detalhamento visual quando há atraso acumulado), barra de progresso, tier + mensagem, frase de projeção baseada em `computePodiumProjection` (baseada só em `pendingMonth` — só as pendências do mês afetam a Taxa deste período): já no pódio → incentiva ir a 100%; fora do pódio e alcançável → "Preencha N atendimento(s) para atingir Y% e entrar no pódio (3º lugar)"; fora do pódio e não alcançável mesmo preenchendo tudo → mensagem genérica. Botão "Regularizar pendências" (só quando há pendência) rola/destaca a tabela de pendências já existente — não abre modal nem cria lista nova.
