...
- `realizadaIds` = `consultationStatuses` com `consumesPrepaidSession === true && !isAwaitingOutcome`.
- `agendadaIds` = `consultationStatuses` com `isAwaitingOutcome === true` (migration 115 — flag explícita, não mais busca por nome).
...
- Exibido como tabela editável **depois** do card "Meu desempenho" e do ranking/especialidades (não logo após "Agenda de Hoje") — de propósito, pra uma lista grande de pendências não empurrar esses painéis pra fora da primeira dobra. Tem `ref` (`pendingSectionRef`) para o botão "Regularizar pendências" do card "Meu desempenho" rolar até ela e destacá-la temporariamente (`ring-2 ring-amber-400` por ~1.5s) — não é uma lista duplicada, é a mesma tabela.
...
- **`src/utils/dashboardMetrics.js`** — helpers puros, única fonte para as duas visões:
  ...
  - `computePodiumProjection({ completed, total, pending, therapistId, rankingSorted })` — quantos preenchimentos faltam para **entrar no pódio** (top 3 do ranking), não para uma faixa arbitrária de 85/95% (a meta real é sempre 100%; o pódio é só o próximo marco acionável no caminho até lá). `rankingSorted` é a lista completa já ordenada (mesma da tabela); a função remove o próprio terapeuta dela (`others`), simula `completed + x` (x de 1 até `pending`) e recalcula a posição exata substituindo-o de volta na lista com o **mesmo** `compareTherapistPerformance` do ranking — `targetPosition` é sempre a posição prevista real (ex.: "1º lugar"), nunca uma suposição genérica de "pelo menos 3º". **Sem prometer o impossível**: se nem preenchendo todas as `pending` bastar pro pódio, `achievable: false` e a UI cai numa mensagem genérica. Se o terapeuta já estiver no pódio (`myPosition <= 3`, o que inclui o caso de menos de 3 terapeutas elegíveis), `onPodium: true` direto, sem passar pelo loop.
  - `compareTherapistPerformance(a, b)` — ordenação única do ranking: maior Taxa → maior **Atendidos** (desempate por volume: quem atendeu mais fica melhor posicionado com a mesma Taxa) → maior Total → nome (pt-BR). Usada tanto para ordenar a tabela quanto para achar a posição do usuário logado e o 3º lugar (pódio).
...
- **`MyPerformanceCard`** (idem, só na view terapeuta; recebe `rankingSorted` inteiro de `DashboardPage.jsx`, o mesmo array usado na tabela): Taxa, Posição (índice+1 na lista ordenada/filtrada da RPC), "{completed} de {total}" (rótulo **"Atendidos"**), Pendências (`pendingMonth + pendingPrevious`, com detalhamento visual quando há atraso acumulado), barra de progresso, tier + mensagem, frase de projeção baseada em `computePodiumProjection` (baseada só em `pendingMonth` — só as pendências do mês afetam a Taxa deste período): já no pódio → incentiva ir a 100%; fora do pódio e alcançável → "Preencha N atendimento(s) para atingir Y% e entrar no pódio em **Zº lugar**" (posição exata, calculada, não "3º" fixo); fora do pódio e não alcançável mesmo preenchendo tudo → mensagem genérica. Botão "Regularizar pendências" (só quando há pendência) rola/destaca a tabela de pendências já existente — não abre modal nem cria lista nova.
- **Auto-refresh a cada 5 minutos** (`setInterval` num `useEffect` próprio, limpo no unmount) — mantém os painéis atualizados numa aba aberta por muito tempo. **Corrigido junto:** o `useEffect` que dispara `reloadMonthlyMetrics()` dependia do objeto `user` inteiro; como `AuthContext.loadUser()` cria um `user` novo a cada evento do Supabase Auth (inclusive `TOKEN_REFRESHED`, silencioso e periódico em background), os painéis recarregavam a cada refresh de token, não só quando o usuário realmente mudava — a dependência passou a ser `user?.authId` (estável), tanto neste efeito quanto no de auto-refresh.
- Busca via `supabase.rpc('get_dashboard_monthly_metrics', {...})` direto em `DashboardPage.jsx` (mesmo padrão local já usado para `financial`/`newLeadsCount`, sem passar pelo `DataContext`) — dispara uma vez (independe de `effectiveView`, reaproveitado pelos dois painéis no toggle dual-role), de novo a cada fechamento do modal de pendência, e a cada 5 minutos. Erro → `Toast` + estado local isolado com botão "Tentar novamente"; nunca derruba o resto do Dashboard.
- **Guarda contra `consultationStatuses` vazio:** o efeito que dispara a busca só roda quando `!dataLoading` (`isLoading` do `DataContext`), não só `!!user`. Sem essa guarda, se a sessão de auth resolvia antes do `fetchAll` do `DataContext` terminar, `realizadaIds`/`agendadaIds` vinham vazios (`consultationStatuses = []` ainda) — e a RPC trata array vazio em `NOT (status = ANY(p_agendada_status_ids))` como sempre `TRUE`, contando **todo mundo como Atendidos**: a tela mostrava 100% pra todos os terapeutas por um instante, até o efeito rodar de novo com os ids corretos assim que `consultationStatuses` carregava (o "pisca 100% e depois corrige" reportado). `PodiumLoading` (corredor até a linha de chegada) substitui o spinner enquanto isso — cobre tanto essa espera quanto o carregamento normal da RPC.
- Componente de loading (`PodiumLoading`, local em `DashboardPage.jsx`) exibe `public/corredor-cruzando-linha-chegada.gif` (384×384 original, 80px na tela) — substituiu uma primeira tentativa em CSS puro (emoji + `@keyframes`) que ficou com o corredor de costas e sobrepondo a bandeira; o GIF pronto resolveu os dois problemas de uma vez.
...
- **Status inicial de novo atendimento** (`ConsultationFormModal`, `!isEdit`): sempre o status `is_awaiting_outcome = true` ativo alfabeticamente primeiro (migration 118 — antes era busca textual por `name.includes('agendada')`, último resquício de busca por nome migrado para flag). **Não** usa `is_scheduling_default` — essa flag é exclusiva do status inicial da **reposição** (`schedulingDefaultStatus`, seção "Reposição de Atendimentos"); reaproveitá-la aqui fazia qualquer atendimento novo nascer com o status configurado pra reposição (bug real, corrigido).
- **`consultation_status_id` é `NOT NULL`** em `consultations` desde a migration 118 — antes era nullable (`ON DELETE SET NULL`), e atendimentos "órfãos" (sem status, originados de exclusão física de um status em uso) faziam o Total do painel "Terapeutas — mês" não bater com Atendidos + Pendências (`status = ANY(array)` retorna `NULL`, não `FALSE`, quando o status é nulo — a linha some das duas contagens sem aparecer em nenhuma). A migration faz backfill dos órfãos existentes para o mesmo status usado como fallback acima, depois aplica a constraint. Efeito colateral desejado: excluir fisicamente um status ainda em uso por algum atendimento passa a falhar em vez de zerar a referência silenciosamente.
