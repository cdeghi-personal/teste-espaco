...
- Componente de loading (`PodiumLoading`, local em `DashboardPage.jsx`) e a animação (`.podium-runner`/`@keyframes podium-run`) ficam em `src/index.css` — único CSS puro do projeto fora do Tailwind, por não haver utilitário nativo pra uma animação de posição em loop.
