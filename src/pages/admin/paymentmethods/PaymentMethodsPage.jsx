...
export default function PaymentMethodsPage() {
  ...
  const filtered = paymentMethods.filter(pm =>
    showInactive ? pm.active === false : pm.active !== false
  )
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="primary" onClick={() => { setEditPM(null); setShowModal(true) }}>
            <FiPlus size={16} /> Nova Forma de Pagamento
          </Button>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Forma de Pagamento</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(pm => {
                ...
                return (
                  <tr key={pm.id} className={`hover:bg-gray-50/50 ${isInactive ? 'opacity-60' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        <button
                          onClick={() => { setEditPM(pm); setShowModal(true) }}
      ...
      {showModal && (
        <PaymentMethodFormModal onClose={() => setShowModal(false)} initial={editPM || {}} />
      )}
    </div>
  )
}
