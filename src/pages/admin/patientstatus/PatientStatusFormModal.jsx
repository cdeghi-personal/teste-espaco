...
export default function PatientStatusFormModal({ onClose, initial = {} }) {
  const { addPatientStatus, updatePatientStatus } = useData()
  ...
  function handleSave() {
    ...
    if (isEdit) updatePatientStatus(initial.id, form)
    else addPatientStatus(form)
    onClose()
  }

  return (
    <Modal
      title={isEdit ? 'Editar Status' : 'Novo Status do Paciente'}
      onClose={onClose}
      size="sm"
      footer={
        <>
    ...
    >
      <div className="space-y-4">
        <Input
          ...
          error={errors.name}
          placeholder="Ex: Ativo, Alta, Pausa..."
        />
        ...
        <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
          ...
          />
          <label htmlFor="ps-active" className="text-sm font-medium text-gray-700">Status ativo</label>
        </div>
      </div>
    </Modal>
  )
