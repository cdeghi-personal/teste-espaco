...
import { FiArrowLeft, FiPlus, FiTrash2, FiFileText, FiDownload } from 'react-icons/fi'
...
import { generateRelatórioConvenioPDF, generateListaPresencaPDF, formatMesLabel, MONTHS } from '../../../utils/generateConvenioPDF'
...
export default function ConvenioReportPage() {
  const {
    patients, therapists, consultations, guardians,
    specialtiesData, diagnoses,
  } = useData()
  const { user } = useAuth()
  ...
  // ── Sessões ──────────────────────────────────────────────────
  const [sessions, setSessions] = useState([])  // [{id, date, value}]
  ...
  function buildCommonParams() {
    ...
    return { patient, specialtyLabel, credential }
  }

  async function handleGenerateRelatorio() {
    const { patient, specialtyLabel, credential } = buildCommonParams()
    if (!patient || !selectedTherapist) return
    setLoadingPDF('relatorio')
    try {
      await generateRelatórioConvenioPDF({
  ...
  async function handleGenerateLista() {
    const { patient, specialtyLabel, credential } = buildCommonParams()
    if (!patient || !selectedTherapist) return
    setLoadingPDF('lista')
    try {
  ...
  const totalSessions = sessions.filter(s => s.date).length
  ...
  const canGenerate = searched && patientId && specialty && selectedTherapist && sessions.filter(s => s.date).length > 0
    ...
    <div className="p-3 md:p-6 space-y-5 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Gera o Relatório ao Convênio e a Lista de Presença em PDF</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select value={patientId} onChange={e => { setPatientId(e.target.value); setSpecialty(''); setSearched(false) }} className={selectClass} disabled={!therapistId && !isAdmin}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-3">
            ...
            <label className="flex items-center gap-1.5 text-sm cursor-pointer">
              <input type="radio" value="range" checked={periodType === 'range'} onChange={() => setPeriodType('range')} />
              Período (De/Até)
            </label>
          </div>
          {periodType === 'month' ? (
        ...
        <Button variant="primary" onClick={handleSearch}>
          <FiFileText size={15} /> Buscar Atendimentos
        </Button>
      </div>
      ...
      {searched && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">2. Sessões ({totalSessions})</h2>
            {totalSessions > 0 && (
              <span className="text-xs font-semibold text-brand-blue">Total: {fmtCurrency(totalValue)}</span>
            )}
          </div>

          {sessions.length === 0 && (
            <p className="text-sm text-gray-500 py-2">Nenhum atendimento encontrado no período. Adicione sessões manualmente abaixo.</p>
          )}
                    ...
                    <tr key={s.id}>
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          step="0.01"
                          value={s.value}
                          onChange={e => updateSession(s.id, 'value', parseFloat(e.target.value) || 0)}
                      ...
                      <td className="px-2 py-2">
                        <button onClick={() => removeSession(s.id)} className="p-1.5 text-gray-400 hover:text-red-500 transition-colors rounded-lg">
                          <FiTrash2 size={14} />
                        </button>
                      </td>
                    </tr>
      ...
      {searched && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-4">4. Gerar Documentos</h2>
          <div className="flex flex-col sm:flex-row gap-3">
            <Button
              variant="primary"
              onClick={handleGenerateRelatorio}
              disabled={!canGenerate || !!loadingPDF}
            >
              <FiFileText size={15} />
              {loadingPDF === 'relatorio' ? 'Gerando...' : 'Relatório ao Convênio (PDF)'}
            </Button>
            <Button
              variant="secondary"
            ...
            >
              <FiDownload size={15} />
              {loadingPDF === 'lista' ? 'Gerando...' : 'Lista de Presença (PDF)'}
            </Button>
          </div>
          {!canGenerate && (
